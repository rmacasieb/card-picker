...
struct CardDetailView: View {
    ...
    var body: some View {
        ...
        ScrollView {
            VStack(spacing: 20) {
                CardVisualView(card: card, accentColor: accentColor)
                
                MultipliersSectionView(card: card, accentColor: accentColor)
                
                if !card.notes.isEmpty {
                    NotesSectionView(card: card)
                }
            }
            .padding()
        }
...
/// Multipliers list section
struct MultipliersSectionView: View {
    let card: CreditCard
    let accentColor: Color
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            ForEach(card.multipliers, id: \.categoryId) { mult in
                if let cat = CategoryCatalog.find(mult.categoryId) {
                    MultiplierRowView(mult: mult, cat: cat, accentColor: accentColor)
                }
            }
        }
        .padding(16)
...
struct MultiplierRowView: View {
    let mult: CardMultiplier
